...
If an OSRS recipe component is not in the bot's current item data, installation is blocked and the missing components are reported. Facility construction requires a saw and hammer and awards the recipe's Construction XP. Required tools such as the Barrel stand, Captured wind mote and Heart of ithell are not consumed.

### Passive Sailing Actions

During every Sailing trip, the bot automatically:

- Trims the installed sails once per completed 30 seconds, using their tier's XP, and rolls the Soup chance for each trim.
...
Catch probabilities use the OSRS rounded success formula out of 256. Each successful catch grants one Fishing XP drop and one Heron roll, regardless of fish quantity. The Fishing level is recorded when the trip starts.
...
- Hulls, masts, sails, helms and keels are stored per ship, but navigation speed and most structural effects are not represented in trip timings. Cargo holds and crew are not yet modelled.
...
- Trimming uses the installed sail's sourced XP. Live navigation speed effects are not represented by the trip timer.
